...
from ..metrics.vector_metrics import compare_vectors
from ..metrics.image_metrics import compute_nmi, compute_pcc, compute_lab_brightness_psnr
...
class StainAnalyzer:
    ...
    AVAILABLE_METRICS = ['vectors', 'ssim', 'pcc', 'nmi', 'lab_psnr']
    PAIRED_ONLY = {'ssim', 'pcc', 'lab_psnr'}
    ...
    def compare(
        ...
        if ref_img is None:
            raise ValueError("No reference image. Pass one to __init__ or to compare().")

        is_paired = reference is not None

        if reference is not None:
            ref_vectors = (
                estimate_stain_vectors(ref_img)
                if 'vectors' in self.metrics else None
            )
        ...
        result = {'id': image_id} if image_id is not None else {}
        ...
        if 'ssim' in self.metrics and is_paired:
            result['ssim'] = float(structural_similarity(
                ref_img, image, channel_axis=-1, data_range=255,
            ))
    ...
    def save_csv(self, output_dir: Union[str, Path]) -> Path:
